...
Google's audience remains **Testing** according to the last recorded dashboard verification. Production publishing was disabled while branding was incomplete. A fresh September 5, 2026 check of `dictamelo.com`, `/privacy` and `/terms` returned HTTP 200 pages that redirect to parking content. These are not verified product or legal pages. Publish the actual homepage, privacy policy and terms before completing production branding. Do not invent a policy or treat a successful HTTP status as sufficient. Production Google publication is separate from configuring Supabase and allowing a test account.
