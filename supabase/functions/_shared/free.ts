...
export async function requireUser(
  ...
  if (!authorization?.startsWith("Bearer ")) {
    throw new LicenseError(401, "Sign in to use your free audio allowance.");
  }
  ...
  if (
    !user.id || !user.email || !user.email_confirmed_at || user.is_anonymous
  ) {
...
export function freeWavSeconds(bytes: Uint8Array): number {
  ...
  if (bytes.length < 44 || tag(0) !== "RIFF" || tag(8) !== "WAVE") {
    throw new LicenseError(
      400,
      "The free plan accepts WAV recordings up to two minutes. Use Pro or your own key for other files.",
    );
  }
  if (view.getUint32(4, true) + 8 !== bytes.length) {
    throw new LicenseError(400, "Invalid WAV recording.");
  }
  let rate = 0, size = 0, formatSeen = false, dataSeen = false;
  let offset = 12;
  for (; offset + 8 <= bytes.length;) {
    ...
    if (tag(offset) === "fmt ") {
      if (
        formatSeen || length < 16 || view.getUint16(offset + 8, true) !== 1 ||
        view.getUint16(offset + 10, true) !== 1 ||
        view.getUint32(offset + 12, true) !== 16000 ||
        view.getUint32(offset + 16, true) !== 32000 ||
        view.getUint16(offset + 20, true) !== 2 ||
        view.getUint16(offset + 22, true) !== 16
      ) throw new LicenseError(400, "Use a mono 16 kHz PCM WAV recording.");
      formatSeen = true;
      rate = 32000;
    }
    if (tag(offset) === "data") {
      if (dataSeen || length % 2) {
        throw new LicenseError(400, "Invalid WAV recording.");
      }
      dataSeen = true;
      size = length;
  ...
  if (!rate || !size || size / rate > 120.1) {
    throw new LicenseError(
      413,
      "Free recordings can be up to two minutes long.",
    );
  }
  return size / rate;
}

export async function reserveFree(
  db: Pick<Db, "rpc">,
  user: string,
  request: string,
  seconds: number,
): Promise<void> {
  await db.rpc("reserve_free_audio", {
    p_user: user,
    p_request: request,
    p_seconds: seconds,
  });
